...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            if loss is not None:
              dataloader_val.collate_fn = self.smart_batching_collate
              dataloader_val2 = iter(dataloader_val)
              running_val = 0
              for i in range(len(dataloader_val)):
                data = next(dataloader_val2)
                features, labels = data
                labels = labels.to(self._target_device)
                features = list(map(lambda batch: batch_to_device(batch, self._target_device), features))

                val_loss = loss(features,labels).item()
                running_val += val_loss

            print("Train error: {}".format(running_train/steps_per_epoch))
            print("Val error: {}".format(running_val/len(dataloader_val)))
            train_errors.append(running_train/steps_per_epoch)
            val_errors.append(running_val/len(dataloader_val))
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback, loss=None):
